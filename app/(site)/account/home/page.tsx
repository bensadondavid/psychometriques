    ...
    <main className="mx-auto w-full max-w-6xl px-4 pb-12 pt-6 sm:px-6 sm:pt-10 lg:px-8">
      ...
      <section className="mt-6 grid border border-border bg-card sm:grid-cols-[1fr_auto]">
        <div className="flex items-start gap-5 p-6 sm:p-8">
          ...
          <div><p className="text-[0.62rem] font-semibold uppercase tracking-[0.2em] text-[#9b7a48]">Prochaine étape</p><h2 className="mt-2 font-serif text-2xl">Votre entraînement arrive bientôt</h2><p className="mt-2 max-w-xl text-sm leading-6 text-muted-foreground">Les premiers parcours structurés seront accessibles depuis cet espace.</p></div>
        </div>
        <div className="flex items-center border-t border-border px-6 py-5 sm:border-l sm:border-t-0"><span className="flex items-center gap-2 text-sm font-semibold text-primary">10 000+ questions <ArrowUpRight className="size-4" /></span></div>
      </section>
    </main>
  )
}
